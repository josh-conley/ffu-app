...
/**
 * ESPN P19 Data Generation Script
 * 
 * This script processes the ESPN Fantasy Scrapes CSV files for P19 (2019 Premier league)
...
// User mapping based on the Sleeper ID Key and configuration for P19
const userMapping = {
...
// Team name to ESPN username mapping for P19
const teamNameMapping = {
  ...
  'The Well Done Stakes': 'stakes',
  'Atlanta Handsome Devils': 'devils',
...
function getUserByTeamName(teamName) {
  ...
  // For historical consistency, use the original 2019 team name
  return {
    userId: userInfo.sleeperId,
    ffuUserId: userInfo.ffuId,
    teamName: teamName, // Use historical 2019 team name
    abbreviation: teamName.split(' ').map(w => w[0]).join('').substr(0, 4).toUpperCase(),
    isHistorical: false
  };
}
...
function loadCSVData() {
  ...
  const teamsCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Teams P19.original.csv'), 'utf8');
  const matchupsCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Matchups P19.original.csv'), 'utf8');
  const draftCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Draft Results P19.original.csv'), 'utf8');
...
function generateDraftData(draftData) {
  ...
  const allPicks = draftData.map((pick, index) => {
    ...
    return {
      ...
      draftId: 'espn-2019-premier-draft'
    };
  }).filter(Boolean);
  ...
  return {
    ...
    metadata: {
      name: 'ESPN 2019 Premier Draft',
      description: 'Premier League Draft for 2019 season',
...
// Main function to generate P19 data
function generateP19Data() {
  ...
  fs.writeFileSync(outputPath, JSON.stringify(p19Data, null, 2));
  
  console.log(`✅ P19 data generated successfully: ${outputPath}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    console.error('❌ Error generating P19 data:', error.message);
    process.exit(1);
  }
}

export { generateP19Data };