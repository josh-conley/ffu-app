...
/**
 * ESPN N18 Data Generation Script
 * 
 * This script processes the ESPN Fantasy Scrapes CSV files for N18 (2018 National league)
...
// User mapping based on the Sleeper ID Key and configuration for N18
const userMapping = {
  ...
  'twds': { ffuId: 'ffu-h03', sleeperId: 'historical-well-done-stakes' },
  'casa': { ffuId: 'ffu-h08', sleeperId: 'historical-team-team-casa' },
  'flame': { ffuId: 'ffu-h06', sleeperId: 'historical-gingy-flame' },
  'cog': { ffuId: 'ffu-h02', sleeperId: 'historical-speedway-ritual-cog' }
};

// Team name to ESPN username mapping for N18
const teamNameMapping = {
  'Goat Emoji': 'swaggy',
  'Oklahoma Banana Bread': 'bread',
  'Team Dogecoin': 'doge',
  'The Well Done Stakes': 'twds',
  'Stark Direwolves': 'stark',
  'CreamOfTheCrop Macho Men': 'scsi',
...
function loadCSVData() {
  ...
  const teamsCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Teams N18.original.csv'), 'utf8');
  const matchupsCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Matchups N18.original.csv'), 'utf8');
  const draftCSV = fs.readFileSync(path.join(dataPath, 'ESPN Fantasy Scrapes - Draft Results N18.original.csv'), 'utf8');
...
function generateMatchupsByWeek(matchupsData, teamsData) {
  ...
  teamsData.forEach(team => {
    ...
    // No truncated names needed for N18 CSV
  });
...
function generateDraftData(draftData) {
  ...
  const allPicks = draftData.map((pick, index) => {
    ...
    return {
      ...
      draftId: 'espn-2018-national-draft'
    };
  }).filter(Boolean);
  ...
  return {
    ...
    metadata: {
      name: 'ESPN 2018 National Draft',
      description: 'National League Draft for 2018 season',
...
// Main function to generate N18 data
function generateN18Data() {
  ...
  fs.writeFileSync(outputPath, JSON.stringify(n18Data, null, 2));
  
  console.log(`✅ N18 data generated successfully: ${outputPath}`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  try {
    ...
    console.error('❌ Error generating N18 data:', error.message);
    process.exit(1);
  }
}

export { generateN18Data };