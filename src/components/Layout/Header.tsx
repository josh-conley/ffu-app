...
import { Users, Calendar, BarChart3, Award, Menu, X, UserPlus, TrendingUp, Grid3x3 } from 'lucide-react';
...
export const Header = () => {
  ...
  const navItems = [
    ...
    { path: '/h2h-matrix', label: 'H2H Matrix', icon: Grid3x3 },
  ];
            ...
            <nav className="hidden lg:flex space-x-2">
              {navItems.map(({ path, label, icon: Icon }) => (
                <Link
                  key={path}
                  to={path}
                  className={`flex items-center space-x-2 px-4 py-3 text-sm font-bold transition-all duration-300 relative group angular-cut-16 ${location.pathname === path
            ...
            <div className="px-4 py-3 space-y-2">
              {navItems.map(({ path, label, icon: Icon }) => (
                <Link
                  ...
                  className={`flex items-center space-x-3 px-4 py-4 text-base font-bold transition-all duration-300 angular-cut-16 ${location.pathname === path
