...
export const DraftList: React.FC<DraftListProps> = ({ draftData, userMap }) => {
  ...
  const handleMemberFilterChange = (member: string | 'all') => {
    ...
    if (member === 'all') {
      ...
      newParams.set('member', member);
    }

    setSearchParams(newParams);
  };

  // Update URL when position filter changes
  const handlePositionFilterChange = (position: string | 'all') => {
  ...
  useEffect(() => {
    const newSelectedMember = searchParams.get('member') || 'all';
    const newSelectedPosition = searchParams.get('position') || 'all';
    const newSelectedTeam = searchParams.get('team') || 'all';

    if (newSelectedMember !== selectedMember) {
      setSelectedMember(newSelectedMember);
    }
    if (newSelectedPosition !== selectedPosition) {
      setSelectedMember(newSelectedPosition);
    }
    if (newSelectedTeam !== selectedTeam) {
      setSelectedMember(newSelectedTeam);
    }
  }, [searchParams]);

  // Filter picks based on selections
  const filteredPicks = (picks || []).filter(pick => {
    if (selectedMember !== 'all' && pick.userInfo.userId !== selectedMember) return false;
    if (selectedPosition !== 'all' && pick.playerInfo.position !== selectedPosition) return false;
    if (selectedTeam !== 'all' && pick.playerInfo.team !== selectedTeam) return false;
    return true;
  });

  // Get unique members for filter
  const uniqueMembers = Array.from(new Set((picks || []).map(p => p.userInfo.userId)))
    .map(userId => {
      ...
      <div className="flex justify-center mb-6">
        <div className="w-full sm:w-auto sm:max-w-[800px] sm:min-w-[500px]">
          <div className="flex flex-wrap gap-4">
            <div>
              <label htmlFor="member-filter" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                Member
              </label>
              <select
                id="member-filter"
